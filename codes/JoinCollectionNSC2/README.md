...
### 분석목적
주어진 데이터를 해석하고 분석할 목적을 결정 할 수 잇는가
분석 목적에 맞게 목표변수와 그에 영향을 주는 설명변수를 봅아낼 수 잇는가
목표변수와 설명변수의 상관관게를 확인하고 각 상관관게에 맞는 분석법을 적용할 수 잇는가
위릐 3가지를 수행할 능력을 다지는 연습을 목적으로 분석을 진행하게 되엇다

### 분석 진행 순서
DDA = 선택한 설명변수 중 각자 분석을 맡은 컬럼을 추출하고,
...
#### 분석결과 
- 수술여부와 진료과목코드, 본인부담금이 요양급여비용총액에 영향을 미친다는 것을 확인엿음
- 나이대별 요양급여비용총액이 다를 것이라고 생각되어 출생년도를 역으로 게산하여 연령을 추출, 목표변수에 영향을 주는지 분석을 시도해보앗으나, 연령대별 진단질병, 연령별 합병증, 가입한 보험등의 컬럼을 생성하고, 데이터를 분리하여 분석을 진행해야 정확한 분석결과를 도출할 수 잇는 가다로운 컬럼이엿다 시간과 분석능력에 한게를 느기고 추가 작업을 진행하지 못하엿다
- 
당연하게 관게가 잇을 것이라고 생각하는 컬럼들을 추출하여 분석을 진행해 보앗다
실제 데이터
